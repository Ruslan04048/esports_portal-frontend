<template>
  ...
  <div class="post-detail">
    <div class="post-header">
      <img class="dota-icon" :src="post.category === 'dota' ? dotaIcon : csIcon" alt="icon" />
      <h1>{{ post.title }}</h1>
    </div>

    <img v-if="post.image" class="news-image" :src="isUrl(post.image) ? post.image : `http://127.0.0.1:8000/storage/${post.image}`
      " alt="news" />
  ...
  <div v-if="post.comments" class="comments-section">
    ...
    <div v-if="auth.token && auth.user" class="new-comment">
      <textarea v-model="newComment" placeholder="Оставить комментарий…" rows="3"></textarea>
