<template>
  <div class="popular-news">
    ...
    <!-- === Карточки новостей === -->
    <div v-if="filteredPosts.length" v-for="post in filteredPosts" :key="post.id" class="news-card">
      <div class="news-info">
          ...
          <div class="stars">
            <template v-for="star in 5" :key="star">
              <svg xmlns="http://www.w3.org/2000/svg" :class="[
                ...
                " :style="{ cursor: auth.token && auth.user ? 'pointer' : 'default' }" viewBox="0 0 24 24" width="20"
                height="20">
                <path
      ...
      <div class="news-body">
        <router-link :to="`/${post.category}/${post.id}`">
          <img v-if="post.image" class="news-image" :src="isUrl(post.image)
              ? post.image
        ...
        <div class="comments-panel">
          <div v-for="c in post.recent_comments" :key="c.id" class="comment">
            <img class="avatar" :src="(c.user && c.user.avatar_url) ||
          ...
          <div v-if="auth.token && auth.user" class="new-comment">
            <textarea v-model="newBodies[post.id]" placeholder="Оставить комментарий…" rows="2"
              :disabled="Boolean(Number(auth.user.banned))"></textarea>

            <button :disabled="!newBodies[post.id]?.trim() || Boolean(Number(auth.user.banned))"
              @click="submitComment(post.id)">
              Отправить
            </button>

            <p v-if="Boolean(Number(auth.user.banned))" style="color: red; margin-top: 0.3rem">
